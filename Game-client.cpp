		...
		else
		{
			...
			while (!quit)
			{
				...
				else if (gameState == 1)
				{
					if (!nameTaken)
					{
						strcpy(cname, player1name.c_str());

						do
						{
							static int flag = 0;
							bytes_sent = send(cli_fd, &cname, sizeof(cname), 0);
							if (bytes_sent == -1 && flag == 0)
							{
								cout << "PLAYER DATA NOT SENT!\n"
									 << "Trying Again...";
								continue;
							}
							else
							{
								...
								bytes_recvd = recv(cli_fd, &sname, sizeof(sname), 0);
								if (bytes_recvd == -1)
									cout << "COULD NOT ACQUIRE PLAYER INFORMATION!\n" << "\n"
										 << "Trying Again..." << "\n";
								else
									cout << "You have joined " << sname << " for a game of Tic-Tac-Toe." << "\n";
								player2name = sname;
							}
